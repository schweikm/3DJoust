							  ...
							  char *_val )
{
	...
	static const unsigned int n_token = 6;

	static char *token[ n_token ] = { "loc", "dir", "fov", "cstart", "cend", "iponame" };
...
void sio2CameraUpdateFrustum( SIO2camera *_SIO2camera )
{
	...
	sio2CameraGetProjectionMatrix( _SIO2camera );

	sio2CameraGetModelviewMatrix( _SIO2camera );

	c[ 0  ] = _SIO2camera->mat_modelview[ 0  ] * _SIO2camera->mat_projection[ 0  ];
	c[ 1  ] = _SIO2camera->mat_modelview[ 1  ] * _SIO2camera->mat_projection[ 5  ];
	c[ 2  ] = _SIO2camera->mat_modelview[ 2  ] * _SIO2camera->mat_projection[ 10 ] +
			  _SIO2camera->mat_modelview[ 3  ] * _SIO2camera->mat_projection[ 14 ];
	
	c[ 3  ] = _SIO2camera->mat_modelview[ 2  ] * _SIO2camera->mat_projection[ 11 ];
	
	c[ 4  ] = _SIO2camera->mat_modelview[ 4  ] * _SIO2camera->mat_projection[ 0  ];
	c[ 5  ] = _SIO2camera->mat_modelview[ 5  ] * _SIO2camera->mat_projection[ 5  ];
	c[ 6  ] = _SIO2camera->mat_modelview[ 6  ] * _SIO2camera->mat_projection[ 10 ] +
			  _SIO2camera->mat_modelview[ 7  ] * _SIO2camera->mat_projection[ 14 ];
	
	c[ 7  ] = _SIO2camera->mat_modelview[ 6  ] * _SIO2camera->mat_projection[ 11 ];

	c[ 8  ] = _SIO2camera->mat_modelview[ 8  ] * _SIO2camera->mat_projection[ 0  ];
	c[ 9  ] = _SIO2camera->mat_modelview[ 9  ] * _SIO2camera->mat_projection[ 5  ];
	c[ 10 ] = _SIO2camera->mat_modelview[ 10 ] * _SIO2camera->mat_projection[ 10 ] +
			  _SIO2camera->mat_modelview[ 11 ] * _SIO2camera->mat_projection[ 14 ];

	c[ 11 ] = _SIO2camera->mat_modelview[ 10 ] * _SIO2camera->mat_projection[ 11 ];
	
	c[ 12 ] = _SIO2camera->mat_modelview[ 12 ] * _SIO2camera->mat_projection[ 0  ];
	c[ 13 ] = _SIO2camera->mat_modelview[ 13 ] * _SIO2camera->mat_projection[ 5  ];
	c[ 14 ] = _SIO2camera->mat_modelview[ 14 ] * _SIO2camera->mat_projection[ 10 ] +
			  _SIO2camera->mat_modelview[ 15 ] * _SIO2camera->mat_projection[ 14 ];

	c[ 15 ] = _SIO2camera->mat_modelview[ 14 ] * _SIO2camera->mat_projection[ 11 ];
	...
	t = 1.0f / sqrtf( _SIO2camera->frustum[ 0 ][ 0 ] *
					  _SIO2camera->frustum[ 0 ][ 0 ] + 
					  _SIO2camera->frustum[ 0 ][ 1 ] *
					  _SIO2camera->frustum[ 0 ][ 1 ] +
					  _SIO2camera->frustum[ 0 ][ 2 ] *
					  _SIO2camera->frustum[ 0 ][ 2 ] );
	...
	t = 1.0f / sqrtf( _SIO2camera->frustum[ 1 ][ 0 ] *
					  _SIO2camera->frustum[ 1 ][ 0 ] +
					  _SIO2camera->frustum[ 1 ][ 1 ] * 
					  _SIO2camera->frustum[ 1 ][ 1 ] +
					  _SIO2camera->frustum[ 1 ][ 2 ] *
					  _SIO2camera->frustum[ 1 ][ 2 ] );
	...
	t = 1.0f / sqrtf( _SIO2camera->frustum[ 2 ][ 0 ] *
					  _SIO2camera->frustum[ 2 ][ 0 ] + 
					  _SIO2camera->frustum[ 2 ][ 1 ] * 
					  _SIO2camera->frustum[ 2 ][ 1 ] + 
					  _SIO2camera->frustum[ 2 ][ 2 ] * 
					  _SIO2camera->frustum[ 2 ][ 2 ] );
	...
	t = 1.0f / sqrtf( _SIO2camera->frustum[ 3 ][ 0 ] *
					  _SIO2camera->frustum[ 3 ][ 0 ] +
					  _SIO2camera->frustum[ 3 ][ 1 ] *
					  _SIO2camera->frustum[ 3 ][ 1 ] +
					  _SIO2camera->frustum[ 3 ][ 2 ] *
					  _SIO2camera->frustum[ 3 ][ 2 ] );
	...
	t = 1.0f / sqrtf( _SIO2camera->frustum[ 4 ][ 0 ] *
					  _SIO2camera->frustum[ 4 ][ 0 ] +
					  _SIO2camera->frustum[ 4 ][ 1 ] *
					  _SIO2camera->frustum[ 4 ][ 1 ] +
					  _SIO2camera->frustum[ 4 ][ 2 ] *
					  _SIO2camera->frustum[ 4 ][ 2 ] );
	...
	t = 1.0f / sqrtf( _SIO2camera->frustum[ 5 ][ 0 ] *
					  _SIO2camera->frustum[ 5 ][ 0 ] + 
					  _SIO2camera->frustum[ 5 ][ 1 ] * 
					  _SIO2camera->frustum[ 5 ][ 1 ] + 
					  _SIO2camera->frustum[ 5 ][ 2 ] * 
					  _SIO2camera->frustum[ 5 ][ 2 ] );
...
void sio2CameraMove( SIO2camera *_SIO2camera,
					 SIO2window *_SIO2window,
					 char  		 _dir )
{
	if( !_SIO2camera->_btRigidBody )
	{
		float speed = _dir * ( _SIO2camera->speed * sio2->_SIO2window->d_time );

		vec3 v;

		v.x = _SIO2camera->_SIO2transform->loc->x +
			  _SIO2camera->_SIO2transform->dir->x +
			  ( _SIO2camera->_SIO2transform->dir->x * speed );
		
		v.y = _SIO2camera->_SIO2transform->loc->y +
			  _SIO2camera->_SIO2transform->dir->y +
			  ( _SIO2camera->_SIO2transform->dir->y * speed );
		
		v.z = _SIO2camera->_SIO2transform->loc->z + 
			  _SIO2camera->_SIO2transform->dir->z;
		
		_SIO2camera->_SIO2transform->loc->x = _SIO2camera->_SIO2transform->loc->x + _SIO2camera->_SIO2transform->dir->x * speed;
		_SIO2camera->_SIO2transform->loc->y = _SIO2camera->_SIO2transform->loc->y + _SIO2camera->_SIO2transform->dir->y * speed;
	...
	{
		_SIO2camera->_btRigidBody->setActivationState( ACTIVE_TAG );
		
		_SIO2camera->_btRigidBody->setLinearVelocity( btVector3( ( _dir * _SIO2camera->_SIO2transform->dir->x ) * _SIO2camera->speed,
																 ( _dir * _SIO2camera->_SIO2transform->dir->y ) * _SIO2camera->speed,
																   _SIO2camera->_btRigidBody->getLinearVelocity()[2] ) );
	}
}


void sio2CameraStrafe( SIO2camera *_SIO2camera,
					   SIO2window *_SIO2window,
					   char  	   _dir )
{
	vec3 v;
	...
	if( !_SIO2camera->_btRigidBody )
	{
		float speed = _dir * ( _SIO2camera->speed * _SIO2window->d_time );
		...
		_SIO2camera->_SIO2transform->loc->x = _SIO2camera->_SIO2transform->loc->x +  _SIO2camera->_SIO2transform->dir->y * speed;
		_SIO2camera->_SIO2transform->loc->y = _SIO2camera->_SIO2transform->loc->y + -_SIO2camera->_SIO2transform->dir->x * speed;
	...
	else
	{
		...
		memcpy( &v, _SIO2camera->_SIO2transform->loc, 12 );
		
		v.x -= _SIO2camera->_SIO2transform->loc->x +  _SIO2camera->_SIO2transform->dir->y * _dir;
		v.y -= _SIO2camera->_SIO2transform->loc->y + -_SIO2camera->_SIO2transform->dir->x * _dir;
		v.z -= _SIO2camera->_SIO2transform->loc->z;
		
		_SIO2camera->_btRigidBody->setLinearVelocity( btVector3( -v.x * _SIO2camera->speed,
																 -v.y * _SIO2camera->speed,
																 _SIO2camera->_btRigidBody->getLinearVelocity()[2] ) );			
	}
}


void sio2CameraElevate( SIO2camera *_SIO2camera,
						SIO2window *_SIO2window,
					    char  		_dir )
{
	float speed = _dir * ( _SIO2camera->speed * _SIO2window->d_time );

	_SIO2camera->_SIO2transform->loc->z = _SIO2camera->_SIO2transform->loc->z + speed;
}


void sio2CameraGetProjectionMatrix( SIO2camera *_SIO2camera )
{
	glGetFloatv( GL_PROJECTION_MATRIX, _SIO2camera->mat_projection ); 
...
{
	glGetFloatv( GL_MODELVIEW_MATRIX, _SIO2camera->mat_modelview );
...
void sio2CameraRender( SIO2camera *_SIO2camera )
{
	...
	if( _SIO2camera->_SIO2ipo && _SIO2camera->_SIO2ipo->state == SIO2_IPO_PLAY )
	{
	...
	else if( _SIO2camera->_btRigidBody )
	{
		...
		memcpy(  _SIO2camera->_SIO2transform->loc, 
				&_SIO2camera->_SIO2transform->mat[ 12 ], 12 );
	...
	v.x = v.y = 0.0f;
	_SIO2camera->_SIO2transform->mat[ 15 ] = v.z = 1.0f;
									 ...
									 float	     _rad )
{
	...
	while( i != 6 )
	{
		d = _SIO2camera->frustum[ i ][ 0 ] * _vec->x + 
			_SIO2camera->frustum[ i ][ 1 ] * _vec->y + 
			_SIO2camera->frustum[ i ][ 2 ] * _vec->z + 
