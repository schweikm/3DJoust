				    ...
				    SIO2stream *_SIO2stream )
{
	...
	if( sio2StringTok( tmp, ".JPG"  ) ||
		sio2StringTok( tmp, ".JPEG" ) )
	{
		sio2ImageLoadJPEG( _SIO2image,
						   _SIO2stream );
	}
	
	else if( sio2StringTok( tmp, ".TGA" ) )
	{
		sio2ImageLoadTGA( _SIO2image,
						  _SIO2stream );
	}
	
	else if( sio2StringTok( tmp, ".PNG" ) )
	{
					 ...
					 float		   _filter )
{
	...
	if( !_SIO2image->tid )
	{
		...
		if( sio2IsEnabled( _flags, SIO2_IMAGE_CLAMP ) )
		{
		...
		if( !sio2IsEnabled( _flags, SIO2_IMAGE_MIPMAP ) )
		{
		...
		{
			if( sio2->tfilter == SIO2_IMAGE_LINEAR )
			{
				glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
				glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
			}
			
			else if( sio2->tfilter == SIO2_IMAGE_BILINEAR )
			{
				glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST );
					 ...
					 unsigned int  _height )
{
	unsigned int i,
				 j,
				 offset,
				 *px = ( unsigned int * ) malloc( _width  * 4 ),
				 *py = ( unsigned int * ) malloc( _height * 4 );
	...
	for( i = 0, tx = 0.0f; i < _width; tx += ratio_x, i++ )
	{ px[ i ] = ( unsigned int )tx; }
	
	for( i = 0, tx = 0.0f; i < _height; tx += ratio_y, i++ )
	{ py[ i ] = ( unsigned int )tx; }
...
void sio2ImageRender( SIO2image *_SIO2image )
{
	...
	glGetTexEnvfv( GL_TEXTURE_FILTER_CONTROL_EXT,
				   GL_TEXTURE_LOD_BIAS_EXT, &f );
	
	if( _SIO2image->filter != f )
	{
...
void sio2ImageReset( unsigned char _channel )
{
