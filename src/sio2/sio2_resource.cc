...
SIO2resource *sio2ResourceInit( void )
{
...
unsigned int sio2ResourceGetVRAMSize( SIO2resource *_SIO2resource )
{
	...
	while( i != _SIO2resource->n_image )
	{
		...
		if( sio2IsEnabled( _SIO2image->flags, SIO2_IMAGE_MIPMAP ) )
		{
								 ...
								 unsigned char  _state )
{
	if( _SIO2resource )
	{
		...
		while( i != _SIO2resource->n_widget )
		{
			...
			if( !sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_VISIBLE ) || 
				!sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_ENABLED ) )
			{
				++i;
				continue;
			}
			else
			{
						  ...
						  _SIO2window->touch[ j ].y < _SIO2widget->tr->y ) )
					{
						...
						switch( _type )
						{
							case SIO2_WINDOW_TAP:
							{
								...
								{
									if( sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_VALIDATE_LOCK ) )
									{
										if( sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_LOCKED ) )
										{
				...
				if( focused && !sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_FOCUSED ) )
				{ sio2EnableState( &_SIO2widget->flags, SIO2_WIDGET_FOCUSED ); }
				
				else if( !focused && sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_FOCUSED ) )
							   ...
							   SIO2object   *_SIO2object )
{
	...
	{
		_SIO2object->_SIO2instance = sio2ResourceGet( _SIO2resource,
													 SIO2_OBJECT,
	...
	while( i != _SIO2resource->n_material )
	{
			...
			!_SIO2material->_SIO2soundbuffer->bid[ 1 ] )
		{
			sio2SoundBufferGenId( _SIO2material->_SIO2soundbuffer,
								  sio2IsEnabled( _SIO2material->sflags, SIO2_SOUND_STREAM ) );
		}
						 ...
						 int		   _mask )
{
	...
	if( _mask & SIO2_RENDER_IPO )
	{
		...
		while( i != _SIO2resource->n_ipo )
		{
			...
			if( _SIO2ipo->state == SIO2_IPO_PLAY )
	...
	if( ( _mask & SIO2_RENDER_LAMP ) && glIsEnabled( GL_LIGHTING ) )
	{
	...
	if( _mask & SIO2_RENDER_ALPHA_OBJECT )
	{
		unsigned int j,
					 n_transp = 0;
		...
		while( i != _SIO2resource->n_object )
		{
			...
			if( _SIO2object->dst > 0.0f && ( _SIO2object->type & SIO2_OBJECT_ALPHA ) )
			{
				++n_transp;
	...
	if( _mask & SIO2_RENDER_SOLID_OBJECT || _mask & SIO2_RENDER_ALPHA_OBJECT )
	{ sio2ObjectReset(); }
								  ...
								  vec2		   *_v )
{
	...
	while( i != _SIO2resource->n_object )
	{
		...
		sio2SetColorIndex( selection, _SIO2object->col );
...
void sio2ResourceResetState( void )
{
