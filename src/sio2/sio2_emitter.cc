...
SIO2emitter *sio2EmitterFree( SIO2emitter *_SIO2emitter )
{
	...
	_SIO2emitter->dir = sio2Vec3Free( _SIO2emitter->dir );
...

{
	...
	memcpy( _SIO2emitter->dir, _dir, 12 );
						...
						SIO2window  *_SIO2window )
{
	...
	if( !glIsEnabled( GL_POINT_SPRITE_OES ) )
	{ glEnable( GL_POINT_SPRITE_OES ); }

	if( !glIsEnabled( GL_COLOR_ARRAY ) )
	{ glEnableClientState( GL_COLOR_ARRAY ); }

	if( !glIsEnabled( GL_POINT_SIZE_ARRAY_OES ) )
	{ glEnableClientState( GL_POINT_SIZE_ARRAY_OES ); }

	sio2MaterialRender( _SIO2emitter->_SIO2material );


	if( _SIO2emitter->_SIO2ipo && _SIO2emitter->_SIO2ipo->state == SIO2_IPO_PLAY )
	{
		...
		sio2TransformSetMat( _SIO2emitter->_SIO2transform,
	...
	glPushMatrix();
	{
		sio2TransformRender( _SIO2emitter->_SIO2transform );
...
{ _SIO2emitter->state = SIO2_EMITTER_PLAY; }


void sio2EmitterPause( SIO2emitter *_SIO2emitter )
{ _SIO2emitter->state = SIO2_EMITTER_PAUSE; }


void sio2EmitterStop( SIO2emitter *_SIO2emitter ) 
{ _SIO2emitter->state = SIO2_EMITTER_STOP; }
