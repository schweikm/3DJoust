...
void sio2BuildMatrix( vec4 *, vec3 *, vec3 *, float * );

void sio2Rotate2D( vec2 *, float, float, vec2 * );

void sio2Rotate3D( vec3 *, float, float, float, vec3 * );


inline void sio2Vec3Add( vec3 *_v1, vec3 *_v2, vec3 *_v3 )
{
	_v3->x = _v1->x + _v2->x;
	_v3->y = _v1->y + _v2->y;
	_v3->z = _v1->z + _v2->z;
...
inline void sio2Vec3Diff( vec3 *_v1, vec3 *_v2, vec3 *_v3 )
{
	_v3->x = _v1->x - _v2->x;
	_v3->y = _v1->y - _v2->y;
	_v3->z = _v1->z - _v2->z;
...
inline void sio2Vec3Interpolate( vec3 *_v1, vec3 *_v2, float _r, vec3 *_v3 )
{
	float ir = 1.0f - _r;
	
	_v3->x = _v1->x * _r + _v2->x * ir;
	_v3->y = _v1->y * _r + _v2->y * ir;
	_v3->z = _v1->z * _r + _v2->z * ir;	 
}


inline float sio2Magnitude( vec3 *_v1 )
{
...
inline float sio2Distance( vec3 *_v1, vec3 *_v2 )
{
	vec3 v3;
...
inline float sio2Normalize( vec3 *_v1, vec3 *_v2 )
{
...
inline float sio2DotProduct( vec3 *_v1, vec3 *_v2 )
{
	return ( _v1->x * _v2->x ) +
		   ( _v1->y * _v2->y ) +
		   ( _v1->z * _v2->z );
...
inline float sio2DotProductToDeg( vec3 *_v1, vec3 *_v2 )
{
	float v = sio2DotProduct( _v1, _v2 );

	if( v )
	{ return acosf( v ) * SIO2_RAD_TO_DEG; }
	
	return 0.0f;
}


inline void sio2CrossProduct( vec3 *_v1, vec3 *_v2, vec3 *_v3 )
{
