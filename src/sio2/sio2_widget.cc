								...
								unsigned char  _usematrix )
{
	if( sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_VISIBLE ) )
	{
		...
		glPushMatrix();
		{
			unsigned char h =  sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_ALIGNH ),
						  v =  sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_ALIGNV );
			if( h || v )
			{
				vec2 abs;
			...
			if( !_usematrix )
			{ sio2TransformApply( _SIO2widget->_SIO2transform ); }
			else
			{ sio2TransformRender( _SIO2widget->_SIO2transform ); }


			glGetIntegerv( GL_ARRAY_BUFFER_BINDING, &curr );

			if( sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_CENTERED ) )
			{
				if(  curr != sio2->wid[ 1 ] )
				{
										...
										SIO2window *_SIO2window )
{
	...
	if( sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_ALIGNH ) )
	{
	...
	{ 
		_SIO2widget->bl->x = !sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_CENTERED ) ? 
							 _SIO2widget->_SIO2transform->loc->x :
							 _SIO2widget->_SIO2transform->loc->x - h_size_x;
		
		_SIO2widget->tr->x = !sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_CENTERED ) ? 
							 _SIO2widget->_SIO2transform->loc->x + _SIO2widget->area->x :
							 _SIO2widget->_SIO2transform->loc->x + h_size_x;
	}
	
	
	if( sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_ALIGNV ) )
	{ 
	...
	{
		_SIO2widget->bl->y = !sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_CENTERED ) ?
							 _SIO2widget->_SIO2transform->loc->y :
							 _SIO2widget->_SIO2transform->loc->y - h_size_y;
		
		_SIO2widget->tr->y = !sio2IsEnabled( _SIO2widget->flags, SIO2_WIDGET_CENTERED ) ?
