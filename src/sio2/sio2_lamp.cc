							...
							char *_val )
{
	...
	static char *token[ n_token ] = { "type"   , "loc", "dir"   , "col" , "nrg",
									  "dst"    , "fov", "sblend", "att1", "att2",
									  "iponame" }; 
							 ...
							 unsigned char  _index )
{
	...
	int GL_CURR_LIGHT = GL_LIGHT0 + _index;


	if( _SIO2lamp->_SIO2ipo && _SIO2lamp->_SIO2ipo->state == SIO2_IPO_PLAY )
	{
		vec3 v;
	...
	if( !glIsEnabled( GL_CURR_LIGHT ) )
	{ glEnable( GL_CURR_LIGHT ); }
	...
	{ glLightfv( GL_CURR_LIGHT, GL_POSITION, &d[ 0 ] ); }
	else
	{
		memcpy( &l[ 0 ], _SIO2lamp->_SIO2transform->loc, 12 );
		l[ 3 ] = 1.0f;
		
		glLightfv( GL_CURR_LIGHT, GL_POSITION, &l[ 0 ] );
		
		if( _SIO2lamp->type == SIO2_LAMP_SPOT )
		{
			glLightfv( GL_CURR_LIGHT, GL_SPOT_DIRECTION, &d[ 0 ] );
			glLightf ( GL_CURR_LIGHT, GL_SPOT_CUTOFF, _SIO2lamp->fov * 0.5f );
			glLightf ( GL_CURR_LIGHT, GL_SPOT_EXPONENT, 128.0f * _SIO2lamp->sblend );
		}
		else
		{ glLightf( GL_CURR_LIGHT, GL_SPOT_CUTOFF, 180.0f ); }
	}


	glLightf( GL_CURR_LIGHT, GL_CONSTANT_ATTENUATION, 1.0f );
	glLightf( GL_CURR_LIGHT, GL_LINEAR_ATTENUATION, _SIO2lamp->att1 / _SIO2lamp->dst );
	glLightf( GL_CURR_LIGHT, GL_QUADRATIC_ATTENUATION, _SIO2lamp->att2 / ( _SIO2lamp->dst * _SIO2lamp->dst ) );
	...
	if( sio2IsEnabled( _SIO2lamp->flags, SIO2_LAMP_NO_DIFFUSE ) )
	{ glLightfv( GL_CURR_LIGHT, GL_DIFFUSE, &black[ 0 ] ); }
	else
	{ glLightfv( GL_CURR_LIGHT, GL_DIFFUSE, &c[ 0 ] ); }
	

	if( sio2IsEnabled( _SIO2lamp->flags, SIO2_LAMP_NO_SPECULAR ) )
	{ glLightfv( GL_CURR_LIGHT, GL_SPECULAR, &black[ 0 ] ); }
	else
	{ glLightfv( GL_CURR_LIGHT, GL_SPECULAR, &c[ 0 ] ); }
