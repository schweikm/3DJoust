					  ...
					  unsigned int	   _flags )
{
	...
	if( sio2IsEnabled( _SIO2sound->flags, SIO2_SOUND_STREAM ) )
	{	
		alSourceQueueBuffers( _SIO2sound->sid,
							  SIO2_SOUNDBUFFER_DOUBLE_BUFFER,
...
{
	if( sio2IsEnabled( _SIO2sound->flags, SIO2_SOUND_FX ) )
...
void sio2SoundSetFx( SIO2sound *_SIO2sound,
					 ...
					 float		_rad )
{
	if( sio2IsEnabled( _SIO2sound->flags, SIO2_SOUND_AMBIENT ) )
...
void sio2SoundSetDefault( SIO2sound *_SIO2sound )
{
	...
	alSourcef( _SIO2sound->sid, AL_GAIN, 1.0f );
...
{
	unsigned char loop =  sio2IsEnabled( _SIO2sound->flags, SIO2_SOUND_LOOP ) &&
						 !sio2IsEnabled( _SIO2sound->flags, SIO2_SOUND_STREAM ) ? 1 : 0;

	_SIO2sound->state = SIO2_SOUND_PLAY;
...
{
	_SIO2sound->state = SIO2_SOUND_PAUSE;
...
{
	_SIO2sound->state = SIO2_SOUND_STOP;
...
void sio2SoundUpdateState( SIO2sound *_SIO2sound )
{
	...
	{
		case AL_PLAYING:
		{ _SIO2sound->state = SIO2_SOUND_PLAY; break; }

		case AL_PAUSED:
		{ _SIO2sound->state = SIO2_SOUND_PAUSE; break; }
		
		case AL_STOPPED:
		{ _SIO2sound->state = SIO2_SOUND_STOP; break; }
	}
...
void sio2SoundUpdateStream( SIO2sound *_SIO2sound )
{
	...
	if(  sio2IsEnabled( _SIO2sound->flags, SIO2_SOUND_STOP   ) ||
		!sio2IsEnabled( _SIO2sound->flags, SIO2_SOUND_STREAM ) )
	{ return; }
	...
	if( !q )
	{
		...
		if( _SIO2sound->_SIO2soundbuffer->curr == _SIO2sound->_SIO2soundbuffer->bid[ 0 ] )
		{
			sio2SoundBufferStream( _SIO2sound->_SIO2soundbuffer, _SIO2sound->_SIO2soundbuffer->bid[ 0 ] );
			alSourceQueueBuffers( _SIO2sound->sid, 1, &_SIO2sound->_SIO2soundbuffer->bid[ 0 ] );

			sio2SoundBufferStream( _SIO2sound->_SIO2soundbuffer, _SIO2sound->_SIO2soundbuffer->bid[ 1 ] );
			alSourceQueueBuffers( _SIO2sound->sid, 1, &_SIO2sound->_SIO2soundbuffer->bid[ 1 ] );
		}
		else
		{
			sio2SoundBufferStream( _SIO2sound->_SIO2soundbuffer, _SIO2sound->_SIO2soundbuffer->bid[ 1 ] );
		...
		if( !sio2IsEnabled( _SIO2sound->flags, SIO2_SOUND_LOOP ) )
		{ sio2SoundStop( _SIO2sound ); }
		else
