...
{
	if( sio2IsEnabled( _SIO2object->flags, SIO2_OBJECT_SOFTBODY ) )
							 ...
							 SIO2object *_SIO2object )
{
	...
	switch( _SIO2object->bounds )
	{
		...
		case SIO2_PHYSIC_TRIANGLEMESH:
		{
			...
			_SIO2object->_btTriangleMesh = new btTriangleMesh();
			...
			while( i != tmp->n_vgroup )
			{
				...
				while( j != tmp->_SIO2vertexgroup[ i ]->n_ind )
				{
					...
					if( tri_cnt == 3 )
					{
						_SIO2object->_btTriangleMesh->addTriangle( tri[ 0 ],
																   tri[ 1 ],
																   tri[ 2 ], 1 );
						tri_cnt = 0;
					}					
			...
			_btCollisionShape = new btBvhTriangleMeshShape( _SIO2object->_btTriangleMesh, 1, 1 );
		...
		case SIO2_PHYSIC_CONVEXHULL:
		{
			...
			_btShapeHull.buildHull( 0.01f );

			delete _btConvexHullShape;

			_btCollisionShape = new btConvexHullShape( (const btScalar *)_btShapeHull.getVertexPointer(),
																		 _btShapeHull.numVertices() );
	...
	if( _SIO2object->mass )
	{			
		_btCollisionShape->calculateLocalInertia( _SIO2object->mass,
												  _btVector3 );
	}


	_btCollisionShape->setMargin( tmp->margin );

	_SIO2object->_btRigidBody = new btRigidBody( _SIO2object->mass,
	...
	if( sio2IsEnabled( _SIO2object->flags, SIO2_OBJECT_DYNAMIC ) )
	{
		...
		{ _SIO2object->_btRigidBody->setAngularFactor( 0.0f ); }
		
		_SIO2object->_btRigidBody->setDamping( _SIO2object->damp,
											   _SIO2object->rotdamp );
	}
	
	
	if( sio2IsEnabled( _SIO2object->flags, SIO2_OBJECT_NOSLEEPING ) )
	{ _SIO2object->_btRigidBody->setActivationState( DISABLE_DEACTIVATION ); }
	...
	if( tmp->n_vgroup )
	{
		...
		while( i != tmp->n_vgroup )
		{
			...
			{
				_SIO2object->_btRigidBody->setFriction( tmp->_SIO2vertexgroup[ i ]->_SIO2material->friction );
				_SIO2object->_btRigidBody->setRestitution( tmp->_SIO2vertexgroup[ i ]->_SIO2material->restitution );
				break;
			}
	...
	_SIO2object->_btRigidBody->setUserPointer( _SIO2object );
							...
							SIO2object *_SIO2object )
{
	...
	_SIO2object->_btSoftBody = new btSoftBody( &_SIO2physic->_btSoftBodyWorldInfo,
											   n_vert,
											   buf,
											   NULL );
	...
	while( i != tmp->n_vgroup )
	{
		...
		while( j != tmp->_SIO2vertexgroup[ i ]->n_ind )
		{
			_SIO2object->_btSoftBody->appendFace( tmp->_SIO2vertexgroup[ i ]->ind[ j     ],
	...
	if( _SIO2object->margin )
	{ _SIO2object->_btSoftBody->getCollisionShape()->setMargin( _SIO2object->margin ); }

	_btTransform.setIdentity();
	
	_btTransform.setFromOpenGLMatrix( _SIO2object->_SIO2transform->mat );
	
	_SIO2object->_btSoftBody->transform( _btTransform );
	
	btSoftBody::Material *_Material = _SIO2object->_btSoftBody->m_materials[ 0 ];	

	
	if( _SIO2object->linstiff )
	{ _Material->m_kLST = _SIO2object->linstiff; }	

	
	_SIO2object->_btSoftBody->m_cfg.piterations = _SIO2object->piteration;
	
	_SIO2object->_btSoftBody->m_cfg.citerations = _SIO2object->citeration;
	
	if( _SIO2object->shapematch )
	{
		_SIO2object->_btSoftBody->m_cfg.kMT  = _SIO2object->shapematch;
		_SIO2object->_btSoftBody->m_cfg.kDP  = 0.0f;
		_SIO2object->_btSoftBody->m_cfg.kCHR = 1.0f;
		_SIO2object->_btSoftBody->m_cfg.kKHR = 0.8f;
		_SIO2object->_btSoftBody->m_cfg.kSHR = 1.0f;		
		
		_SIO2object->_btSoftBody->setPose( false, true );
	}
	else
	{ _SIO2object->_btSoftBody->setPose( true, false ); }
	...
	if( tmp->n_vgroup )
	{
		...
		while( i != tmp->n_vgroup )
		{
			if( tmp->_SIO2vertexgroup[ i ]->_SIO2material )
			{
				_SIO2object->_btSoftBody->m_cfg.kDF = tmp->_SIO2vertexgroup[ i ]->_SIO2material->friction * 0.01f;
				break;
			}
	...
	_SIO2object->_btSoftBody->prepareClusters( 16 );

	_SIO2object->_btSoftBody->setTotalMass( _SIO2object->mass, true );

	_SIO2object->_btSoftBody->randomizeConstraints();
	
	_SIO2object->_btSoftBody->setUserPointer( _SIO2object );	
	
	_SIO2physic->_btSoftRigidDynamicsWorld->addSoftBody( _SIO2object->_btSoftBody );
}
							...
							SIO2object *_SIO2object )
{
	...
	while( i != _SIO2physic->_btSoftRigidDynamicsWorld->getNumCollisionObjects() )
	{
		...
		if( _SIO2object->_btRigidBody == _btRigidBody )
		{
