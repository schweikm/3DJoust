...
{ _SIO2thread->state = SIO2_THREAD_PLAY; }


void sio2ThreadPause( SIO2thread *_SIO2thread )
{ _SIO2thread->state = SIO2_THREAD_PAUSE; }


void sio2ThreadStop( SIO2thread *_SIO2thread )
{ _SIO2thread->state = SIO2_THREAD_STOP; }
...
void *sio2ThreadRun( void *_ptr )
{
	...
	while( run )
	{
		switch( _SIO2thread->state )
		{
			case SIO2_THREAD_PLAY:
			{
				_SIO2thread->_SIO2threadcallback( _SIO2thread->userdata );
				break;
			}
			
			case SIO2_THREAD_STOP:
			{
				run = 0;
				break;
			}
