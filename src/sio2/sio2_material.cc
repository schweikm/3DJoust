...
SIO2material *sio2MaterialInit( char *_name )
{
							    ...
							    char *_val )
{
	...
	static char *token[ n_token ] = { "tflags0" , "tname0"   , "tfilter0", "tflags1"    , "tname1"  ,
									  "tfilter1", "sflags"   , "sbname"  , "diffuse"    , "specular",
									  "alpha"   , "shininess", "friction", "restitution", "alvl"    ,
									  "blend" }; 
...
void sio2MaterialRender( SIO2material *_SIO2material )
{
	...
	if( _SIO2material->blend )
	{
		...
		if( sio2->_SIO2blend != _SIO2material->blend )
		{ sio2->_SIO2blend = _SIO2material->blend; }
		else
		{ goto alpha_test; }


		switch( _SIO2material->blend )
		{
			case SIO2_MATERIAL_ALPHA:
			{
...
alpha_test:
	...
	if( _SIO2material->alvl )
	{
		...
		glAlphaFunc( GL_GREATER, _SIO2material->alvl );
	}
	else
	{
		if( glIsEnabled( GL_ALPHA_TEST ) )
	...
	glColor4f( _SIO2material->diffuse->x,
			   _SIO2material->diffuse->y,
			   _SIO2material->diffuse->z,
			   _SIO2material->diffuse->w );
